    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10">
            <a href="#how-it-works" className="text-sm font-bold text-slate-500 hover:text-emerald-600 transition-colors">{t('landing.how_it_works')}</a>
            <a href="#solutions" className="text-sm font-bold text-slate-500 hover:text-emerald-600 transition-colors">{t('landing.solutions')}</a>
            <a href="#network" className="text-sm font-bold text-slate-500 hover:text-emerald-600 transition-colors">{t('landing.market_network')}</a>
          </div>

          <div className="hidden md:flex items-center gap-4">
            <LanguageSwitcher />
            <Link to="/login" className="text-sm font-bold text-slate-600 px-6 py-2.5 rounded-xl hover:bg-slate-50 transition-all">{t('landing.log_in')}</Link>
            <Link 
              to="/signup" 
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="relative z-10 space-y-10 animate-in fade-in slide-in-from-left duration-700">
            <div className="inline-flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest">
              ...
              {t('landing.india_leading_exchange')}
            </div>
            <h1 className="text-6xl md:text-7xl font-black text-slate-900 leading-[1.1] tracking-tight">
              {t('landing.direct_sourcing')} <br />
              <span className="text-emerald-600">{t('landing.zero_friction')}</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-5">
              ...
              <div className="flex flex-col justify-center">
                 ...
                 <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('landing.trusted_by_industries')}</p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white p-4 rounded-[3.5rem] border border-slate-100 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] relative z-10">
               <div className="bg-slate-50 p-10 rounded-[2.8rem] space-y-8">
                  <div className="flex items-center justify-between">
                     <h3 className="font-black text-xl text-slate-900 tracking-tight">{t('landing.active_market')}</h3>
                     <div className="flex gap-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t('landing.live_updates')}</span>
                     </div>
                  </div>
                  ...
                  <div className="pt-6 border-t border-slate-200 grid grid-cols-2 gap-4">
                     <div className="text-center p-4 bg-white rounded-2xl border border-slate-100">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('landing.total_vol')}</p>
                        <p className="font-black text-slate-900">14.2k MT</p>
                     </div>
                     <div className="text-center p-4 bg-emerald-600 rounded-2xl">
                        <p className="text-[10px] font-black text-emerald-100 uppercase tracking-widest mb-1">{t('landing.price_index')}</p>
                        <p className="font-black text-white">{t('landing.stable')}</p>
                     </div>
                  </div>
               </div>
            </div>
      ...
      <section className="pb-32 px-6">
         <div className="max-w-7xl mx-auto border-y border-slate-100 py-12 flex flex-wrap items-center justify-center gap-x-20 gap-y-10">
            <div className="text-sm font-black text-slate-300 uppercase tracking-[0.4em]">{t('landing.verified_sourcing')}</div>
            <div className="text-sm font-black text-slate-300 uppercase tracking-[0.4em]">{t('landing.gst_compliant')}</div>
            <div className="text-sm font-black text-slate-300 uppercase tracking-[0.4em]">{t('landing.live_logistics')}</div>
            <div className="text-sm font-black text-slate-300 uppercase tracking-[0.4em]">{t('landing.audit_ready')}</div>
         </div>
      </section>
            ...
            <div className="space-y-8 order-2 lg:order-1">
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="p-8 rounded-3xl bg-slate-50 border border-slate-100 space-y-4 hover:border-emerald-200 transition-colors">
                     ...
                     <h4 className="font-bold text-slate-900">{t('landing.for_farmers')}</h4>
                     <p className="text-sm text-slate-500 leading-relaxed font-medium">{t('landing.farmer_desc')}</p>
                  </div>
                  <div className="p-8 rounded-3xl bg-slate-50 border border-slate-100 space-y-4 hover:border-blue-200 transition-colors">
                     ...
                     <h4 className="font-bold text-slate-900">{t('landing.for_industry')}</h4>
                     <p className="text-sm text-slate-500 leading-relaxed font-medium">{t('landing.industry_desc')}</p>
                  </div>
               </div>
            </div>
            
            <div className="space-y-8 order-1 lg:order-2">
               <h2 className="text-sm font-black text-emerald-600 uppercase tracking-[0.3em]">{t('landing.our_ecosystem')}</h2>
               <h3 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">
                  {t('landing.seamless_supply')}
               </h3>
               ...
               <ul className="space-y-4">
                  {[
                     t('landing.benefit_price'),
                     t('landing.benefit_logistics'),
                     t('landing.benefit_compliance'),
                     t('landing.benefit_payments')
                  ].map((item, i) => (
      ...
      <section id="how-it-works" className="py-32 bg-slate-900 text-white px-6 overflow-hidden relative">
         ...
         <div className="max-w-7xl mx-auto relative z-10">
            <div className="mb-20 space-y-4">
               <h2 className="text-sm font-black text-emerald-400 uppercase tracking-[0.3em]">{t('landing.the_process')}</h2>
               <h3 className="text-4xl md:text-5xl font-black tracking-tight">{t('landing.three_steps')}</h3>
            </div>
      ...
      <footer className="py-20 bg-white border-t border-slate-100 px-6">
         <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
            ...
            <div>
               <h4 className="font-black text-[10px] uppercase tracking-widest text-slate-400 mb-6">{t('common.marketplace')}</h4>
               <ul className="space-y-4 text-sm font-bold text-slate-900">
                  <li><Link to="/signup">{t('landing.for_farmers')}</Link></li>
                  <li><Link to="/signup">{t('landing.for_industry')}</Link></li>
                  <li><Link to="/signup">{t('common.logistics_portal_label')}</Link></li>
               </ul>
            </div>
            <div>
               <h4 className="font-black text-[10px] uppercase tracking-widest text-slate-400 mb-6">{t('landing.company')}</h4>
               <ul className="space-y-4 text-sm font-bold text-slate-900">
                  <li><a href="#">{t('landing.about_us')}</a></li>
                  <li><a href="#">{t('landing.security')}</a></li>
                  <li><a href="#">{t('landing.legal')}</a></li>
               </ul>
            </div>
         </div>
         <div className="max-w-7xl mx-auto mt-20 pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
            <p className="text-xs font-bold text-slate-400">{t('landing.all_rights_reserved')}</p>
