import { Plus, Search, Filter, IndianRupee, Package, Calendar, Edit2, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{t('my_listings')}</h1>
          <p className="text-slate-500 mt-1">{t('total_listings_count', { count: myListings.length })}</p>
        </div>
        <Link 
      ...
      {isListingsLoading ? (
        ...
        <div className="bg-white rounded-3xl p-12 text-center border border-dashed border-slate-200">
          ...
          <h3 className="text-xl font-bold text-slate-900">{t('no_listings_yet')}</h3>
            ...
            <div key={listing.listing_id} className="eco-card overflow-hidden group">
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-bold text-slate-900 line-clamp-1">{listing.category?.name || t('bio_waste')}</h3>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-slate-400 text-xs">{t('per_unit', { unit: listing.category?.unit || 'unit' })}</div>
                  </div>
                </div>

                <div className="flex items-center gap-4 py-3 border-y border-slate-50">
                  <div className="flex-1">
                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">{t('quantity')}</p>
                    <p className="text-sm font-bold text-slate-700">{listing.quantity} {listing.category?.unit}</p>
                  </div>
                  <div className="w-px h-8 bg-slate-100" />
                  <div className="flex-1">
                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">{t('created')}</p>
