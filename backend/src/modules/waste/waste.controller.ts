...
import { wasteCategoryServices, wasteListingServices } from "./waste.services.js";
import {
...
class WasteListingsController {
  create = async (req:Request,res:Response) => {
   try {
    ...
    const newListings = await wasteListingServices.create(result.data,Imgurl,farmer_id,category_id);
  ...
  update = async (req:Request,res:Response) => {
    ...
    const listing = await wasteListingServices.update(result.data,Imgurl,category_id,listing_id);
